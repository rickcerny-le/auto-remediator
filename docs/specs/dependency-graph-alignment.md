# dependency-graph-alignment Specification

## Purpose

Defines how the system reads intra-family dependencies from the feed and computes minimal collateral bumps over the declared family, so that declared siblings are raised to satisfy dependency ranges (escalating beyond policy only when required for correctness) and collateral bumps are surfaced distinctly from primary bumps.
