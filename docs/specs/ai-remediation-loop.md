# ai-remediation-loop Specification

## Purpose

Defines how the system attempts to repair a dependency bump that broke compilation: when a model-backed agent is invoked, how its proposed source edits are constrained and applied to the run's existing working tree, how attempts are bounded and re-verified, and how the whole exchange is disclosed as a transcript artifact and in the pull request. The loop is strictly an improvement on a rejected change — exhausting it SHALL never leave a run worse off than never having tried.
