...
#### Scenario: A repaired change rests in AwaitingReview
- **WHEN** verification rejects the change with compile diagnostics and the remediation loop produces edits that verify
- **THEN** the run passes through `Remediating` and rests in `AwaitingReview` with a proposal recorded, and no push or pull request occurs

#### Scenario: An approved proposal reaches Completed
- **WHEN** a run resting in `AwaitingReview` is approved and its change set is pushed
- **THEN** the run passes through `Pushing` and `CreatingPr`, ends with status `Completed`, and records the pull request link

#### Scenario: A discarded proposal reaches Discarded
- **WHEN** a run resting in `AwaitingReview` is discarded by a reviewer
- **THEN** the run ends with status `Discarded`, and no branch push or pull request occurs
...
#### Scenario: A held run survives a round trip
- **WHEN** a run resting in `AwaitingReview` is persisted and read back
- **THEN** its awaiting-review status and proposal reference are preserved
