...
Each Dockerfile SHALL use a multi-stage build: a .NET 10 SDK stage to restore and publish in Release, and a minimal .NET 10 runtime stage for the final image. Apps (`api`, `web`) SHALL use the ASP.NET Core runtime base and expose the app port; workers (`scheduler`, `remediation`) SHALL use the .NET runtime base and expose no port.
...
#### Scenario: Worker image is a runnable console image
- **WHEN** the `scheduler` or `remediation` image is built
