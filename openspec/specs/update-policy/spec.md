...
Defines how AutoRemediator selects the target version for each matched package from an update policy — applying a per-scope strategy (`Patch`/`Minor`/`Major`) relative to the current pin, holding packages named by an ignore list, and respecting pre-release exclusion — so that dependency analysis and update execution act on a policy target rather than the absolute latest version.
