...
Defines the trunk-based CI/CD pipeline for AutoRemediator: PR validation builds, trunk builds that push service images to ACR, a deploy stage that rolls out the test environment, and the parameterization and reusable structure that keep the pipeline portable and documented.
