...
Defines the remediation run state machine, persisted run history, and per-message idempotency that orchestrate a single remediation run from a `RemediationRunRequested` message to a terminal status.
